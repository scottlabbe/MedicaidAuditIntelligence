...
const get = <T,>(o: any, a: string, b?: string): T | undefined => o?.[a] ?? (b ? o?.[b] : undefined);

export default function ReportDetailTabs({ report }: ReportDetailTabsProps) {
  const [activeTab, setActiveTab] = useState("summary");

  // normalize fields once
  const overallConclusion = get<string>(report, "overallConclusion", "overall_conclusion");
  const auditScope        = get<string>(report, "auditScope", "audit_scope");
  const aiScopeSummary    = get<string>(report, "potentialObjectiveSummary", "potential_objective_summary");
    ...
    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
      <div className="border-b border-border">
        <TabsList className="grid w-full grid-cols-4 h-auto p-0 bg-transparent">
          <TabsTrigger value="summary"         className="border-b-2 border-transparent data-[state=active]:border-primary rounded-none py-4 px-6">Summary</TabsTrigger>
          <TabsTrigger value="objectives"      className="border-b-2 border-transparent data-[state=active]:border-primary rounded-none py-4 px-6">Objectives</TabsTrigger>
          <TabsTrigger value="findings"        className="border-b-2 border-transparent data-[state=active]:border-primary rounded-none py-4 px-6">Findings</TabsTrigger>
          <TabsTrigger value="recommendations" className="border-b-2 border-transparent data-[state=active]:border-primary rounded-none py-4 px-6">Recommendations</TabsTrigger>
        </TabsList>
      </div>
