...
export default function SearchFilters({ filters, onFiltersChange }: SearchFiltersProps) {
  ...
  return (
    <Card className="border warm-shadow rounded-2xl">
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        <CardHeader className="pb-3">
          <CollapsibleTrigger asChild>
            <Button variant="ghost" className="w-full justify-between p-0 focus-ring hover:bg-surface-2 filter-transition">
              <CardTitle className="flex items-center space-x-2 text-primary">
                <Filter className="w-5 h-5 text-secondary" />
                <span>Filters</span>
                {activeFilterCount > 0 && (
                  <span className="bg-orange-light text-orange-primary text-xs px-2 py-1 rounded-full border border-orange-primary/20">
                    {activeFilterCount}
                  </span>
                )}
              </CardTitle>
              <X className={`w-4 h-4 text-muted transition-transform filter-transition ${isOpen ? "rotate-45" : ""}`} />
            </Button>
          </CollapsibleTrigger>
          {activeFilterCount > 0 && (
            <Button
              ...
              className="mt-2 border text-secondary hover:bg-surface-2 focus-ring"
            >
              Clear all
            </Button>
          )}
